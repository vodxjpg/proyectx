...
function parseHexColor(hex: string): [number, number, number] {
  // Remove # and handle shorthand hex (e.g., #fff -> #ffffff)
  ...
  const r = parseInt(c.substring(0, 2), 16) / 255; // Red
  const g = parseInt(c.substring(2, 4), 16) / 255; // Green
  const b = parseInt(c.substring(4, 6), 16) / 255; // Blue
  console.log(`Parsed ${hex} to RGB: [${r}, ${g}, ${b}]`); // Debug log
  return [r, g, b];
}
...
void main() {
    ...
    vec3 cFinal = vec3(0.0, 0.0, 0.0); // Default to transparent black (will be transparent with alpha)
    if (total > 0.0) {
        float alpha1 = m1 / total;
        float alpha2 = m2 / total;
        cFinal = iColor * alpha1 + iCursorColor * alpha2; // Black balls
    }
    outColor = vec4(cFinal * f, f); // Transparent background where no metaballs, opaque where metaballs are
}
`;
